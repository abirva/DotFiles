...
define(['jquery', 'underscore', '/bext/pub/backbone.js', '/util/etask.js',
    ...
    'config', '/util/storage.js', '/www/util/pub/user_navbar.js',
    '/bext/vpn/pub/social.js', 'backbone', '/util/escape.js'],
    function($, _, be_backbone, etask, zerr, B, be_ui_vpn,
    be_ui_svc_require, be_ui_obj, version_util, be_popup_lib,
    be_util, T, be_config, storage, user_navbar, be_social,
    Backbone, zescape){
...
var tabs_view_class = Backbone.View.extend({
    ...
    initialize: function(opts){
        ...
        _.each(tabs, function(tab_opts){
            ...
            var tab = assign({}, tab_opts, {$label: $label,
                $content: $content});
        ...
        if (this.tabs.length<=saved_active || saved_active<0)
            saved_active = 0;
        this.show_tab(this.tabs[saved_active]);
    ...
    get_tab: function(num){ return this.tabs[num]; },
    // tabs is not a real Array - cannot use includes()
    find_tab: function(tab){ return this.tabs.indexOf(tab); }
});
